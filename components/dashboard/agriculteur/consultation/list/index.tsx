...
const ConsultationListComponent = () => {
  ...
  useEffect(() => {
    const fetchFarms = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/api/consultations/farmer/${user?.id}`, {
  ...
  <div className="mt-6 flex flex-col sm:flex-row justify-between gap-4">
    <button
      ...
      className="w-full sm:w-auto bg-dark-green text-white py-2 px-4 rounded-lg hover:bg-dark-green-dark transition duration-300"
    >
      Ajouter une ferme
    </button>
    <button
